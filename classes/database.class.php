<?php 
  class Database{
    ...
    public static function buscar2($seleciona, $parametros=array()){
      $conexao = self::iniciaConexao();
      $stmt = $conexao->prepare($seleciona);
      $stmt = self::vinculaParametros($stmt, $parametros);
      $stmt->execute();
      return $stmt->fetchALL();
    }

    public static function buscar3($seleciona, $parametros=array(), $aluno=array()){
      $conexao = self::iniciaConexao();
      $stmt = $conexao->prepare($seleciona);
      $stmt = self::vinculaParametros($stmt, $parametros, $aluno);
      $stmt->execute();
      return $stmt->fetchALL();
    }
